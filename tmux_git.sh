...
TMUX_OUTREPO_STATUS='not a git repository'
    ...
    if [ -n "$TMUX" ]; then     
        ...
        if [ $TMUX_GIT_LASTREPO ] && [ "$TMUX_GIT_LASTREPO" = "${CWD:0:$LASTREPO_LEN}" ]; then
            ...
            if [ "$GIT_DIRTY" ]; then 
                ...
                tmux set-window-option status-$TMUX_STATUS_LOCATION "#[fg=$TMUX_DIRTY_COLOR]$TMUX_STATUS" > /dev/null            
            else
                tmux set-window-option status-$TMUX_STATUS_LOCATION-attr none > /dev/null
                tmux set-window-option status-$TMUX_STATUS_LOCATION "#[fg=$TMUX_STATUS_COLOR]$TMUX_STATUS" > /dev/null            
            fi
            ...
            if [ "$GIT_REPO" ]; then
                ...
                tmux set-window-option status-$TMUX_STATUS_LOCATION "#[fg=$TMUX_STATUS_COLOR]$TMUX_OUTREPO_STATUS" > /dev/null
